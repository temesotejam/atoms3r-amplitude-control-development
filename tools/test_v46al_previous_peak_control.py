...
assert manifest['version'] in ('0.46.37','0.46.38','0.46.39','0.46.40','0.46.41')
assert 'V46al / 0.46.37' in site or 'V46am / 0.46.38' in site or 'V46an / 0.46.39' in site or 'V46ao / 0.46.40' in site or 'V46ap / 0.46.41' in site
...
int main(){
 ...
 auto early=evaluate(7.5f,8.0f,1,8.0f,9999);assert(!early.applied&&early.reason==PREV_REASON_BEFORE_ENABLE_TIME&&early.corrected_free_peak_deg==7.5f);
 auto other=evaluate(7.5f,8.0f,1,10.0f,15000);assert(!other.applied&&other.reason==PREV_REASON_TARGET_UNSUPPORTED);
 auto outside=evaluate(7.5f,6.0f,1,8.0f,15000);assert(!outside.applied&&outside.reason==PREV_REASON_OUTSIDE_SUPPORT);
 ...
 auto bad=evaluate(NAN,8.0f,1,8.0f,15000);assert(!bad.applied&&bad.reason==PREV_REASON_INVALID_INPUT&&std::isnan(bad.corrected_free_peak_deg));
}
'''
